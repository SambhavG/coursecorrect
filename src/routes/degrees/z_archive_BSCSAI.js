...
function BSCSAILUT() {
  return {
      ...
      'CS 161': "^CS 161$",
      'A': "^CS 221$",
      'B1': '^CS (224R|228|229|229M|229T|234|238)$',
      'B2': '^CS (124|224[NSUV])$',
      'B3': '^CS (131|231A|231N)$',
      'B4': '^CS (123|223A|237A)$',
      'C AI Methods': '^(CS (157|205L|230|236|257)|STATS 315[AB])$',
      'C Comp Bio': '^CS (235|279|371)$',
      'C Info & Web': '^CS (224W|276)$',
      'C Ethics': '^CS 256$',
      'C Robotics & Control': '^(CS (225A|3277A|329)|ENGR 205|MS&E [23]51)$',
      'C Other': '^CS (151|227B)$',
      'Track electives': "^(CS (325B|326|329D|330|428)|EE (263|28|364[AB])|MS&E (252|352|355)|PHIL 152|PSYCH (204[AB]|209)|STATS (20(0235)|271))$",
...
function BSCSAI(allCourses, grid, list, transfer) {
  ...
  let a = CourseRegexMatch(allCourses, lut['A']);
  let b1Lut = CourseRegexMatch(allCourses, lut['B1']);
  let b2Lut = CourseRegexMatch(allCourses, lut['B2']);
  let b3Lut = CourseRegexMatch(allCourses, lut['B3']);
  let b4Lut = CourseRegexMatch(allCourses, lut['B4']);
  let cAIMethodsLut = CourseRegexMatch(allCourses, lut['C AI Methods']);
  let cCompBioLut = CourseRegexMatch(allCourses, lut['C Comp Bio']);
  let cInfoWebLut = CourseRegexMatch(allCourses, lut['C Info & Web']);
  let cEthicsLut = CourseRegexMatch(allCourses, lut['C Ethics']);
  let cRoboticsControlLut = CourseRegexMatch(allCourses, lut['C Robotics & Control']);
  let cOtherLut = CourseRegexMatch(allCourses, lut['C Other']);
  ...
  //Compute A
  ret = extractAndCreateCells(listCopy, a, 1);
  let aCourse = ret.courses[0];
  listCopy = ret.courseList;

  //Compute B
  let bLuts = [b1Lut, b2Lut, b3Lut, b4Lut];
  let b = [];
  bLuts.forEach((lut) => {
    if (b.length < 2) {
      ret = extractAndCreateCells(listCopy, lut, 1);
      if (ret.wasFound) {
        b.push(ret.courses[0]);
        listCopy = ret.courseList;
      }
    }
  });
  ...
  //Compute C
  let cLuts = bLuts.concat([cAIMethodsLut, cCompBioLut, cInfoWebLut, cEthicsLut, cRoboticsControlLut, cOtherLut]);
  let c = [];
  cLuts.forEach((lut) => {
    if (c.length < 1) {
      ret = extractAndCreateCells(listCopy, lut, 1);
    ...
    rows: [
      {
        cells: [
          {
            value: 'B.S. in Computer Science (AI Track)',
            isTitle: true,
            noBorder: true
          }
        ]
